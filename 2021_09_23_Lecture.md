...
first line of code show the original names of the df second line use
janitor to convert the names to snake cases, replacing the old version
of the df with the new version, the one cleaned by janitor
`janitor::clean_names(litters_df)` is a short way to tell r to find the
clean\_names function in janitor without having to load all of the
janitor package
...
There are multiple arguments in `read_csv`, such as skip, na, etc. which
can be helpful to clean the data, for example:
...
LotR Words is next, the data set is not nicely organized, we need to
import cells B3-D6
...
read.csv always load everything as a data frame instead of a tibble, it
shows the entire result but does not show information about the \# of
rows or columns. Tibble is printed out more nicely But most of the other
reasons are things in the background.
